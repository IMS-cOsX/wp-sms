...
/*
add autoload.php file path or database path in the path '...' section.
*/
// require_once(plugin_dir_path(__FILE__) . '...');
// //add name of clint name at 'example'
// use example\Rest\Client;
...
class Sendex_Admin
{
     ...
     */
		add_options_page(
			...
			'manage_options',
			$this->plugin_name,
			array($this, 'display_sendex_settings_page')
		);
	}
	...
	public function send_message()
	{
		...
		/*
		this section is for added api details from the database
		this section is for added api details from the database
		this section is for added api details from the database
		this section is for added api details from the database
		*/

		//add api details from the database.
		$api_details = get_option('sendex'); #sendex is use to identify options, it can be anything

		if (is_array($api_details) and count($api_details) != 0) {
			$example_SID = $api_details['api_sid'];
			$example_TOKEN = $api_details['api_auth_token'];
		}

		// try {
		// 	$to = explode(',', $to);

		// 	$client = new Client($example_SID, $example_TOKEN);
